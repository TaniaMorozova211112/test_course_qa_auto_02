...
# year_min = search_criteria[0]     рік >= 2017
# engine_min = search_criteria[1]   двигун >= 1.6
# price_max = search_criteria[2]    ціна <= 36000
year_min, engine_min, price_max = search_criteria

# Створюємо пустий список для машин, які відповідають критеріям
new_list_cars = []

# Проходимо по всіх машинах у словнику
for car_name, details in car_data.items():
    color = details[0]
    year = details[1]
    ...
    # Перевіряємо, чи машина відповідає всім трьом критеріям
    if year >= year_min and engine_volume >= engine_min and price <= price_max:
      new_list_cars.append((car_name, details))

# Сортуємо список знайдених машин за зростанням ціни
new_list_cars.sort(key=lambda x: x[1][4])

# Виводимо 5 перших машин
print("Автомобілі, які відповідають критеріям пошуку:")
for car in new_list_cars[:5]:
    print(f"'{car[0]}': {car[1]}")
